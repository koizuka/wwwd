...
procedure Register;
begin
  RegisterComponents('Koizuka', [TTrayIcon]);
end;
...
procedure TTrayIcon.OnMouseTrayDown(Button: TMouseButton;
...
procedure TTrayIcon.OnMouseTrayIcon(var Message: TMessage);
var
  pt: TPoint;
begin
  GetCursorPos(pt);
...
procedure TTrayIcon.SetIconHandle(const Value: HICON);
begin
...
function TTrayIcon._DoNotify(cmd: DWORD): BOOL;
begin
{$IFDEF NOTIFYICON_WIDECHAR}
...
begin
  if TrayList.Items[Message.wParam] <> nil then
    (TObject(TrayList.Items[Message.wParam]) as TTrayIcon).OnMouseTrayIcon(Message);
end;
  ...
  if Message.Msg = WM_QUERYENDSESSION then begin
    for i := 0 to TrayList.Count-1 do
      if TrayList.Items[i] <> nil then
        (TObject(TrayList.Items[i]) as TTrayIcon).Hide;
  end;
  inherited;
end;
...
begin
  for i := 0 to TrayList.Count-1 do
    if TrayList.Items[i] <> nil then
      with TObject(TrayList.Items[i]) as TTrayIcon do begin
        Hide;
        Apply;
      end;
end;
