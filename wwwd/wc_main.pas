unit wc_main;
{TODO:DNSタイムアウトなどもエラー種別表示に追加したい}
...
{TODO:トレイアイコンのシングルクリック/ダブルクリックの効果的な識別(一応タイマーでやってあるが)}
{TODO:チェック対象のスマート推奨機能。rdfがあるときはそれを見つけるとか。ItemPropertyに分析ボタン? 次回分析チェックも? }
...
{TODO:イベントログ}
{TODO:なんらかの複数PC共有手段}
{TODO:RSS, Atomを読んでサブアイテムとして各記事を更新判断?}
{TODO:チェック開始後、自動的に開くはずなのに開かないことがあるbug}
{TODO:Dateフィールドがタイムゾーン変更に対応していない}
{TODO:自動オープンチェックボックスをツールバーに付ける。チェックの最中に解除できるようにしたい}
{TODO:Firefox1.0.2でDDEタイトル取得が化ける件。UTF8下位バイトしか来てないの?}
{TODO:wwwd.dat merge(via network?)}
{TODO:wwwd.dat/Dateフィールド見直し。画面のDate/Size/LastModified, OrgDate,OrgSize見直し。すべての情報を.datに継承したい}
{TODO:未読を開いた日時を覚え、その後チェックしたときに前回チェックより更新していても、開いた時刻より前だったら更新にしない?}
{TODO:タイトル取得で開くURLを読んだときに、チェックURLと違ってても更新時刻などが変化してしまってるbug fix}
{TODO:最小化したときにタスクバーから消えないオプション}
...
アイテムのチェックURLは履歴つけるかね。
 ...
 チェック時のRDF対応も欲しい。
 <?xml version="1.0"...>で囲まれた領域で
  <rdf:RDF xmlns="http://purl.org/rss/1.0/"
  xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
   (このへん要確認)
   の中で
   <channel rdf:....>
    <title>タイトル<title> <-これからタイトル取得できるよな
    <items>...
   <item rdf:...>
    <dc:date>更新日時<dc:date> <-複数のアイテムの更新日時を全部チェックして、一番新しいものを採用するか?
      なおこの日時formatはRFC822の場合もあるが 2005-05-11T02:24:Z みたいなものもあるようだ。
      ISO 8601(W3CDTF:http://www.w3.org/TR/NOTE-datetime)で日付だけの場合もある。
      存在しない場合はファイル自体のタイムスタンプで?


参考:
  RDF http://www.w3.org/RDF/
  Atom format http://www.mnot.net/drafts/draft-nottingham-atom-format-02.html
}
...
uses
  ...
  Options,
  WwwdData,
  DropSource, DropTarget, DragDrop, DropURLListTargetV4,
  AsyncHttp, httpAuth, AsyncSockets;
...
type
  ...
  THttpControl = record
    ...
    rawSize: integer;
    proxyAuthIndex: integer;
  end;
  PHttpControl = ^THttpControl;
  THttpProxyAuthInfo = record
    proxyAuth: THttpAuth;
    proxyAuthIdentity: string;
    assignedHttp: TAsyncHttp;
  end;
  PHttpProxyAuthInfo = ^THttpProxyAuthInfo;
  ...
  private
    ...
    FProxyAuths: array of THttpProxyAuthInfo;
    FOptions: TOptions;
    FData: TWwwdData;
    FStop: boolean;
    FDirty: boolean;
    ...
    function OpenItem(CheckItem: TCheckItem; manual: boolean): boolean;
    ...
    procedure LaunchProgram(programname, params, directory: string);
    ...
    procedure SetAllowAutoOpen(const Value: boolean);
    function GetProxyAuthInfo(var htps: THttpControl; var auth: THttpAuth): boolean;
    function StartAuthProxy(var htps: THttpControl): boolean;
    function GenerateAuthProxy(var htps: THttpControl; method, uri, send_body: string): string;
    procedure UpdateAuthProxy(var htps: THttpControl);
    function SplitProxyAuthInfo(fullname: string; var user, password,
      hostname: string): boolean;
    function GetHttps(const http: TAsyncHttp): PHttpControl;
    function AllocateProxyAuth: integer;
    procedure ClearProxyAuth;
    procedure AssignProxyAuth(authinfo: string;
      var htps: THttpControl; var index: integer);
    procedure DisconnectProxyAuth(var htps: THttpControl; remove: boolean);
  protected
    procedure WndProc(var Message: TMessage); override;
  public
    { Public 宣言 }
    function OpenBrowsers( const items: TWwwdCheckItemContainer; option: TBrowserOptions; manual: boolean ): integer;
    ...
    property AllowAutoOpen: boolean read FAllowAutoOpen write SetAllowAutoOpen;
...
begin
  ...
  if SortColumn = SortByName1.Tag then
    ...
    Item := SortByComment1
  else if SortColumn = SortByTouch.Tag then
    Item := SortByTouch
  else
    Item := SortByPriority;
...
begin
  ...
  begin
    ...
    for i := LastNum-1 downto newSimul do
    begin
      DisconnectProxyAuth(https[i], true);
...
begin
  if AllowAutoOpen and FOptions.AutoOpen then
  begin
  ...
  repeat
    ...
    begin
      ...
      begin
        ...
        if GetProxyAuthInfo( GetHttps(http)^, proxy_auth) then
          extraheaders := extraheaders + GenerateAuthProxy( GetHttps(http)^, methods[RequestType], url, '' );
...
begin
  ...
  if (responseCode = HttpResult407_ProxyAuthenticationRequired) and StartAuthProxy(htps^) then
  begin
    CheckItem.Retry;
    // 認証応答を生成したので再度接続 /
  end
...
begin
  ...
  begin
    ...
    DisconnectProxyAuth(htps^, false);
...
procedure TWWWDForm.ListView1Change(Sender: TObject; Item: TListItem;
  Change: TItemChange);
var
  lv: TCheckItemViewListItem;
begin
  if Change = ctState then
  begin
    lv := TCheckItemViewListitem.FromListItem(Item);
    if lv <> nil then
      if lv.CheckItem <> nil then
        if lv.CheckItem.Priority <> Item.Checked then
        begin
          lv.CheckItem.Priority := Item.Checked;
          SetDirty;
        end;
    PostUpdateSelectStatus;
  end;
...
procedure TWWWDForm.ListView1Compare(Sender: TObject; Item1, Item2: TListItem;
  ...
  begin
    ...
    if SortColumn >= Item1.SubItems.Count then begin
      if SortColumn = SortByTouch.Tag then
      begin
      ...
      if SortColumn = SortByPriority.Tag then
      begin
        b1 := TCheckItemViewListItem.FromListItem(Item1).CheckItem.Priority;
        b2 := TCheckItemViewListItem.FromListItem(Item2).CheckItem.Priority;
        if b1 > b2 then
          result := -1
        else if b1 = b2 then
          result := 0
...
begin
  ...
  priority := FCurrentGroup.UpdatePriorityCount > 0;
  ...
  try
    ...
    try
      ...
      begin
        ...
        if (CheckItem.Priority = priority) and CheckItem.IsToBeOpen then
        begin
      ...
      opencount := OpenBrowsers(opens, [], false);
      if opencount > 0 then
      begin
...
begin
  ...
  try
    GetSelected( sl );
    if sl.Count > 0 then
      OpenBrowsers(sl, [boOpenNew], true);
  finally
    sl.Free;
  end;
end;
...
begin
  ...
  AllowAutoOpen := false;
  if FOptions.OpenNewBrowser then
...
procedure TWWWDForm.LaunchProgram(programname, params, directory: string);
var
...
begin
  ...
  if Pos(' ', progname) > 0 then
    if progname[1] <> '"' then
      progname := '"' + progname + '"';
  if not CreateProcess( PChar(programname), PChar(progname + ' ' + params), nil, nil, false, 0, nil, PChar(directory), startupinfo, procinfo ) then
  begin
...
function TWWWDForm.OpenBrowsers( const items: TWwwdCheckItemContainer;
  option: TBrowserOptions; manual: boolean ): integer;
var
  i: integer;
  params: string;
  CheckItem: TCheckItem;
...
begin
  ...
  begin
    ...
    if FOptions.FireFoxCommandLine then
    begin
      quote := '"';
      delimiter := '|';
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        LaunchProgram(FOptions.GetLaunchProgramName, quote + params + quote, FOptions.ProgramPath);
      end else begin
        opt := option;
...
begin
  ...
  try
    ...
    try
      sl.AddCheckItem(ci);
      result := OpenBrowsers(sl, [boOpenNew], false) > 0;
    finally
      sl.Free;
    end;
  finally
...
function TWWWDForm.OpenItem(CheckItem: TCheckItem; manual: boolean): boolean ;
var
  sl: TItemList;
begin
  sl := TItemList.Create;
  try
    sl.AddCheckItem( CheckItem );
    result := OpenBrowsers(sl, [], manual) > 0;
  finally
    sl.Free;
  end;
end;
...
begin
  https[slot-1].busy := 0;
  DisconnectProxyAuth(https[slot-1], newIcon = ErrorIcon);
...
begin
  ...
  numUnreadPriority := FCurrentGroup.UpdatePriorityCount;
  updated := numUnreadPriority > 0; //numUnread > 0;
  NextAction.Enabled := numUnread > 0;
  SelectNew1.Enabled := numUnread > 0;

  lead := FCurrentGroup.GetDisplayName;

  if AllowAutoOpen then
    lead := '+' + lead;

  if numUnread > 0 then
  begin
    if numUnreadPriority > 0 then
      lead := lead + ' ' + Format(NumUnreadPriorityLabel, [numUnreadPriority, numUnread - numUnreadPriority])
    else
...
function TWWWDForm.GetHttps( const http: TAsyncHttp ): PHttpControl;
begin
  result := nil;
  if (http.Tag <= 0) or (http.Tag > Length(https)) then
    Exit;
  if https[http.Tag-1].http <> http then
    Exit;
  result := @https[http.Tag-1];
end;

function TWWWDForm.AllocateProxyAuth: integer;
var
  lastsize: integer;
begin
  lastsize := Length(FProxyAuths);
  SetLength(FProxyAuths, lastsize + 1);
  with FProxyAuths[lastsize] do
  begin
    proxyAuth := THttpAuth.Create(true);
    proxyAuthIdentity := '';
    assignedHttp := nil;
  end;
  result := lastsize;
...
procedure TWWWDForm.ClearProxyAuth;
var
  i: integer;
begin
  for i := 0 to Length(FProxyAuths) - 1 do
  begin
    with FProxyAuths[i] do
    begin
      proxyAuth.Free;
      proxyAuthIdentity := '';
      assignedHttp := nil;
  ...
  SetLength(FProxyAuths, 0);
end;

procedure TWWWDForm.DisconnectProxyAuth( var htps: THttpControl; remove: boolean );
var
  i: integer;
begin
  if htps.proxyAuthIndex < 0 then
    Exit;

  i := htps.proxyAuthIndex;
  if (i < 0) or (i >= Length(FProxyAuths)) then
  begin
    // 不整合
    htps.proxyAuthIndex := -1;
    Exit;
  end;

  if FProxyAuths[i].assignedHttp <> htps.http then
  begin
    // 不整合
    htps.proxyAuthIndex := -1;
    Exit;
  ...
  FProxyAuths[i].assignedHttp := nil;
  if remove then
    FProxyAuths[i].proxyAuthIdentity := '';
end;

procedure TWWWDForm.AssignProxyAuth( authinfo: string; var htps: THttpControl; var index: integer);
var
  i: integer;
  freepos: integer;
begin
  ...
  for i := 0 to Length(FProxyAuths)-1 do
  begin
    if (freepos < 0) and (FProxyAuths[i].proxyAuthIdentity = '') then
      freepos := i;
    if FProxyAuths[i].proxyAuthIdentity = authinfo then
      if FProxyAuths[i].assignedHttp = nil then
      begin
        htps.proxyAuthIndex := i;
        FProxyAuths[i].assignedHttp := htps.http;
        index := i;
        exit;
      end;
  end;
  ...
  begin
    index := freepos;
    FProxyAuths[index].proxyAuth.Init;
  end
  else
    index := AllocateProxyAuth;
  htps.proxyAuthIndex := index;
  FProxyAuths[index].assignedHttp := htps.http;
  FProxyAuths[index].proxyAuthIdentity := authinfo;
end;

function TWWWDForm.SplitProxyAuthInfo( fullname: string; var user, password, hostname: string ): boolean;
var
  i: integer;
begin
  result := false;
...
function TWWWDForm.GetProxyAuthInfo( var htps: THttpControl; var auth: THttpAuth ): boolean;
var
  auth_id: string;
...
begin
  ...
  auth := nil;
  result := False;
  if not http.UseProxy then
    Exit;

  if not SplitProxyAuthInfo(http.Proxy, user, password, hostname) then
    Exit;
  if user = '' then
    Exit;

  auth_id := http.Proxy + ':' + IntToStr(http.ProxyPort);
  AssignProxyAuth(auth_id, htps, auth_index);
  auth := FProxyAuths[auth_index].proxyAuth;
  result := True;
end;

function TWWWDForm.StartAuthProxy( var htps: THttpControl ): boolean;
var
  auth: THttpAuth;
begin
  result := false;
  if not GetProxyAuthInfo(htps, auth) then
    Exit;

  result := auth.Start(htps.http);
end;

procedure TWWWDForm.UpdateAuthProxy( var htps: THttpControl );
var
  auth: THttpAuth;
begin
  if not GetProxyAuthInfo(htps, auth) then
  ...
  auth.ReadyForNext(htps.http);
end;

function TWWWDForm.GenerateAuthProxy( var htps: THttpControl; method, uri, send_body: string ): string;
var
  auth: THttpAuth;
  user, password, hostname: string;
begin
  ...
  if not SplitProxyAuthInfo(htps.http.Proxy, user, password, hostname) then
    Exit;

  result := auth.Generate( method, uri, send_body,
    user, password,
    'wwwd' + IntToHex(Integer(auth),8)
  );
end;

procedure TWWWDForm.SetAllowAutoOpen(const Value: boolean);
begin
  if FAllowAutoOpen = Value then
    Exit;
  FAllowAutoOpen := Value;
  UpdateIconCount;
end;
