...
uses
  ...
  FileCtrl,
  WwwdData,
  DropSource, DropTarget, DropURLListTarget,
...
type
  ...
  private
    ...
    procedure SetTrayIcon(i: TTrayIconType);
...
begin
  RegisterGroup( FData.CreateUniqueGroupName( NewGroupName ) ).View.EditText;
end;
...
begin
  ...
  http1.UserAgent := ProgramName + '/1.00 (' + Version + '; Win32)';
...
begin
  ...
  begin
    ...
    if HeaderDialog.CheckItem = CheckItem then
      HeaderDialog.RawBuffer := HeaderDialog.RawBuffer + receivedData;
    receivedData := htps.htmlSize.Append(receivedData);
...
procedure TWWWDForm.SetTrayIcon( i: TTrayIconType);
var
  newTip:string;
begin
  newTip := Copy(Application.title,1,63);
  TrayIcon1.Tip := newTip;
  TrayIcon1.IconHandle := FTrayIcons[i];
end;
...
begin
  if Button = mbLeft then begin
    ...
    if FTrayClickTime < nowtime then begin
