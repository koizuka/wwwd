unit ItemProperty;
{TODO:複数選択時もURL共通部分を有効にしたい}

{ Control.Tagは、0だと編集可能、1だと"(複数選択)"で編集不可 }
...
procedure TItemPropertyDlg.UpdateCheckItem( CheckItem: TCheckItem );
  ...
  function TestCheck( const CheckBox: TCheckBox ): boolean;
  begin
    result := CheckBox.State <> cbGrayed;
  end;
  procedure CheckCheck( var dest: boolean; CheckBox: TCheckBox );
  begin
    if TestCheck(CheckBox) then
...
begin
  ...
  if TestCheck(PriorityCheck1) then
    CheckItem.Priority := Prioritycheck1.Checked;
...
begin
  ...
  UpdateReason.Caption := UpdateReasonsToStr(CheckItem.UpdateReasons);
  LabelCrc.Caption := CheckItem.OrgCrc;
  LabelETag.Caption := CheckItem.ETag;
  PriorityCheck1.Checked := CheckItem.Priority;
  FItemCount := 1;
  SetMultiMode(false);
end;
...
begin
  ...
  CheckCheck( PriorityCheck1, CheckItem.Priority );
  CheckSoon1.Checked := false;
