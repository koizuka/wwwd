...
begin
  ...
  begin
    result := '';
    result := result + CmdTable[cmdCaption] + ': ' + Caption + #13#10;
    result := result + CmdTable[cmdUrl] + ': ' + CheckUrl +#13#10;
    if Updated then
      result := result + CmdTable[cmdModified] + ': ' + UpdateReasonsToStr(UpdateReasons) + #13#10;
    if OrgDate <> '' then
    ...
    if CreateDate <> 0 then
      result := result +  CmdTable[cmdCreateDate] + ': '+ EncodeRFC822Date(CreateDate)+#13#10;
    if LastCheckDate <> 0 then
      result := result +  CmdTable[cmdLastCheckDate] + ': '+ EncodeRFC822Date(LastCheckDate)+#13#10;
    if SkipIt then
    ...
    if Priority then
      result := result +  CmdTable[cmdPriority] + ': Yes' + #13#10;
    result := result + #13#10;
  end;
end;
...
begin
  FItems.Add( CheckItem );
  TouchItem( CheckItem );
  if CheckItem.CreateDate = 0 then
    CheckItem.UpdateCreateDate;
    ...
    if Copy(line, 1, 6) = '<NAME>' then begin
      ...
      if Copy(line, 1, 5) = '<URL>' then begin
        ...
        if s[Length(s)] = '*' then begin
          ...
          CheckItem.SetUpdated(True, [urDate]);
        ...
        if s[Length(s)] = '*' then begin
          SetLength( s, Length(s)-1 );
          CheckItem.SetUpdated(True, [urSize]);
        end;
        CheckItem.OrgSize := s;
...
begin
  ...
  begin
    ...
    if buf[cmdModified] <> '' then
      SetUpdated(True, StrToUpdateReasons(buf[cmdModified]));
    ...
    if buf[cmdLastCheckDate] <> '' then begin
      try
        ...
        try
          LastCheckDate := DecodeRFC822Date(buf[cmdLastCheckDate]);
        except
          LastCheckDate := 0.;
        end;
      end;
    end;
    if buf[cmdCreateDate] <> '' then begin
      try
        CreateDate := DecodeRFC822Date(buf[cmdCreateDate]);
      except
        CreateDate := 0.;
      end;
    end;
    ...
    if buf[cmdManualOpenCount] <> '' then
      if StrToIntDef(buf[cmdManualOpenCount], 0) > 0 then
        Priority := True;
    if buf[cmdPriority] <> '' then
      Priority := True;
  end;
end;
